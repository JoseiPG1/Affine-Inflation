...
\begin{align}
  ...
  C \nabla^\mu \nabla_\mu \phi - \Lambda \mathcal{V}'(\phi) & = 0
\end{align}
where the Klein-Gordon field equation emerge naturally from the parallelism constraint. 

The second type of solution its given by \textit{Harmonic field equation} which is coupled with a kinetic term and potential energy
\begin{equation}
  \nabla_{[\mu}\left(\mathcal{R}_{\nu]\gamma}\frac{1}{\mathcal{V}(\phi)}\right) 
  - C \nabla_{[\mu}\left(\partial_{\nu]} \phi \partial_\gamma \phi \frac{1}{\mathcal{V}(\phi)}\right) = 0
\end{equation}
...
\begin{equation}
  a_f(t) = 3\kappa \cosh^2(t\sqrt{A_1})
\end{equation}
...
\section{GR as a subspace of PAG}
