...
that the affine function $h(t) = h_0$ must be a constant. This type of approuch lead to a system of differential equations for a given potential $\mathcal{V}(\phi)$
that can be solved and additionally, in this solution the Ricci tensor it is not covariantly constant, however it is degenerate, meaning that the spatial part is trivial. 
